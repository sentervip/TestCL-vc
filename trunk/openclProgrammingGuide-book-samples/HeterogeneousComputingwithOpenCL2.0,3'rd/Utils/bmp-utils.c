 ...
 */
float* readBmp_u8(const char *filename, int* rows, int* cols) {
   ...
   *rows = height;    
   ...
   float* intImage = NULL;
   intImage = (float*)malloc(sizeof(float)*width*height);
   if(intImage == NULL) {
      perror("malloc");
      exit(-1);
   }
   ...
   for(i = 0; i < height; i++) {
      for(j = 0; j < width; j++) {
         intImage[i*width+j] = (float)imageData[i*width+j];
      }
   }
 ...
 * Read bmp image and convert to byte array. Also output the width and height
 */
float* readBmp(const char *filename, int* rows, int* cols) {
   ...
   *rows = height;    
   ...
   // Input image on the host
   float* floatImage = (float *) malloc(sizeof(float)*width*height);
   if(floatImage == NULL) {
      perror("malloc failed bmp");
      exit(-1);
   }
