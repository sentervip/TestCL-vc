...
const sampler_t s_repeat = CLK_FILTER_NEAREST | CLK_NORMALIZED_COORDS_FALSE | CLK_ADDRESS_NONE;
           ...
           __global float4 *out)
{
    ...
    /* repeat */
    out[6] = read_imagef(im, s_repeat, (float2)(0.0f,0.0f));
    out[7] = read_imagef(im, s_repeat, (float2)(1.0f,2.0f));
    out[8] = read_imagef(im, s_repeat, (float2)(1.0f,3));
}
